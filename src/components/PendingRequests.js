...
const PendingRequests = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get("https://pausedesk.byethost18.com/get_leaves.php?role=hod");
  ...
  const handleAction = async (id, status) => {
    ...
    try {
      const res = await axios.post("https://pausedesk.byethost18.com/update_leave_status.php", {
        id,
        status,
        hod_reason: reason,
